const EMBEDDED_VIEW = new URLSearchParams(location.search).get("embedded") === "1";
const EMBEDDED_SECTION = new URLSearchParams(location.search).get("view");
if (EMBEDDED_VIEW) {
  document.body.classList.add("embedded-view");
  if (EMBEDDED_SECTION) document.body.classList.add(`embedded-${EMBEDDED_SECTION}`);
}
...
const DOCUMENT_MAPS = {
  financial: ["customs", "interchange", "streets"],
  pmc: ["reserve", "lighthouse", "icebreaker"],
  project: ["factory", "customs", "reserve"],
  blueprints: ["factory", "interchange", "labyrinth"],
  test: ["woods", "shoreline", "icebreaker"],
  user: ["streets", "ground-zero", "laboratory"],
  medical: ["ground-zero", "laboratory", "labyrinth"],
  technical: ["woods", "shoreline", "lighthouse"]
};
...
function renderMapDocumentSymbols() {
  elements.mapSelector.querySelectorAll("button[data-map]").forEach(button => {
    ...
    categoryKeys.filter(key => DOCUMENT_MAPS[key].includes(button.dataset.map)).forEach(key => {
...
function renderTabs() {
  elements.pageTabs.innerHTML = PAGES.map(page => `<button type="button" data-page="${page.number}" class="page-tab ${page.number === selectionState.page ? "active" : ""}" aria-current="${page.number === selectionState.page ? "page" : "false"}"><span>${t("progress.pageNumber", { number: page.number })}</span><small>${t("progress.pageRequired", { count: page.total })}</small></button>`).join("");
}
...
function renderDocumentBelt(requirements = remainingRequirements()) {
  elements.documentBelt.innerHTML = categoryKeys.map(key => {
    ...
    const mapNames = DOCUMENT_MAPS[key].map(map => t(`maps.${MAP_TRANSLATION_KEYS[map]}`)).join(" · ");
    return `<div class="progress-document-item ${needed ? "active" : "complete"}" data-document="${key}" style="--color:${category.color}" title="${t(`documentTitles.${key}`)}: ${needed}">
      <i class="document-icon"></i><span class="document-count"><small>${category.code}</small><b>${needed}</b></span><span class="document-meta"><strong>${t(`documentTitles.${key}`)}</strong><small>${mapNames}</small></span>
    </div>`;
  }).join("");
}
