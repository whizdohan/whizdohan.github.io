...
const MAPS=[
  {id:"customs",name:"Customs",code:"CST",file:"customs-2d.jpg",width:7680,height:4025,svg:"Customs.svg",credit:"Shebuka, thaddeus"},
  {id:"ground-zero",name:"Ground Zero",code:"GZ",file:"ground-zero-2d.jpg",width:6920,height:6920,svg:"GroundZero.svg",credit:"Shebuka"},
  {id:"factory",name:"Factory",code:"FCT",file:"factory-2d.jpg",width:6720,height:3328,svg:"Factory.svg",credit:"Shebuka, thaddeus, Hanzik"},
  {id:"woods",name:"Woods",code:"WDS",file:"woods-2d.jpg",width:6994,height:6843,svg:"Woods.svg",credit:"Shebuka, thaddeus, Razzmatazzz, Hanzik"},
  {id:"reserve",name:"Reserve",code:"RSV",file:"reserve-2d.jpg",width:4701,height:2785,svg:"Reserve.svg",credit:"Shebuka, Razzmatazzz, thaddeus, LuccaAnthoine, TheGlu3guy"},
  {id:"shoreline",name:"Shoreline",code:"SHR",file:"shoreline-2d.jpg",width:6668,height:4567,svg:"Shoreline.svg",credit:"Shebuka, Razzmatazzz, thaddeus"},
  {id:"interchange",name:"Interchange",code:"INT",file:"interchange-2d.jpg",width:2400,height:1350,source:"https://reemr.se/interchange/",sourceLabel:"RE3MR",credit:"RE3MR and in-map contributors"},
  {id:"lighthouse",name:"Lighthouse",code:"LHT",file:"lighthouse-2d.jpg",width:2242,height:3892,svg:"Lighthouse.svg",credit:"Shebuka, Razzmatazzz, thaddeus"},
  {id:"streets",name:"Streets of Tarkov",code:"SOT",file:"streets-2d.jpg",width:7620,height:5877,svg:"StreetsOfTarkov.svg",credit:"Shebuka, Razzmatazzz"},
  {id:"laboratory",name:"The Lab",code:"LAB",file:"labs-2d.jpg",width:3820,height:1980,svg:"Labs.svg",credit:"Razzmatazzz, Shebuka"},
  {id:"icebreaker",name:"Icebreaker",code:"ICE",file:"icebreaker-2d.jpg",width:2400,height:1350,source:"https://reemr.se/icebreaker/",sourceLabel:"RE3MR",credit:"RE3MR, Ency, Ereshkigal, MTDV, Malrods, RakTheGoose, Muhawi, AllHost.io"},
  {id:"labyrinth",name:"The Labyrinth",code:"LBY",file:"labyrinth-2d.jpg",width:2400,height:2160,source:"https://reemr.se/labyrinth/",sourceLabel:"RE3MR",credit:"RE3MR, Muhawi, RakTheGoose, Ency, Airwingmarine, AllHost.io"}
].map(map=>({...map,image:`${MAP_ASSET_ROOT}${map.file}?v=${MAP_ASSET_VERSION}`,source:map.source||`https://github.com/the-hideout/tarkov-dev-svg-maps/blob/main/${map.svg}`,sourceLabel:map.sourceLabel||"tarkov.dev SVG Maps"}));
...
const els={
  ...
  mapSource:document.querySelector("#map-source"),mapCredit:document.querySelector("#map-credit"),fallbackImage:document.querySelector("#map-image-fallback"),
...
function prepareStaticMap(){
  ...
  els.fallbackImage.src=activeMap.image;
}
...
function renderMapDocumentFilters(){
  ...
  const documentCountsByCategory=new Map(Object.keys(CATEGORIES).map(key=>[key,0]));
  mapLocations.forEach(item=>documentCountsByCategory.set(item.category,(documentCountsByCategory.get(item.category)||0)+1));
  const available=Object.keys(CATEGORIES).filter(key=>documentCountsByCategory.get(key));
  ...
  els.mapDocumentFilters.innerHTML=available.map(key=>{
    ...
    const documentCount=documentCountsByCategory.get(key);
    return `<button type="button" class="map-document-filter ${enabled?"active":""}" data-map-filter="${key}" aria-pressed="${enabled}" title="${label}: ${documentCount}"><img src="${documentIconUrl(key)}" alt="" /><span>${label}</span><b>${documentCount}</b></button>`;
  }).join("");
}
