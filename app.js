const MAPS=[
  ["customs","Customs","세관","CST"],["ground-zero","Ground Zero","그라운드 제로","GZ"],
  ["factory","Factory","공장","FCT"],["woods","Woods","삼림","WDS"],
  ["reserve","Reserve","리저브","RSV"],["shoreline","Shoreline","해안선","SHR"],
  ["interchange","Interchange","인터체인지","INT"],["lighthouse","Lighthouse","등대","LHT"],
  ["streets","Streets of Tarkov","타르코프 시내","SOT"],["laboratory","The Lab","연구소","LAB"],
...
function updateMap(){
  ...
  els.currentMapName.textContent=`${map.name} · ${map.ko}`;els.mapCode.textContent=map.code;els.mapSelect.value=map.id;
  const filtered=locations.filter(location=>location.map===map.id&&state.filters.includes(location.category));
  els.markerLayer.innerHTML=filtered.map(location=>{const category=CATEGORIES[location.category]||CATEGORIES.technical;return `<button class="map-marker" data-id="${location.id}" style="left:${location.x}%;top:${location.y}%;--marker:${category.color}" aria-label="${escapeHtml(location.title||"문서 위치")}"></button>`}).join("");
  els.emptyState.hidden=filtered.length>0;els.visibleCount.textContent=`표시 위치 ${filtered.length}개`;
}
...
function bindEvents(){
  ...
  document.querySelector("#toggle-all").addEventListener("click",event=>{const allEnabled=state.filters.length===Object.keys(CATEGORIES).length;state.filters=allEnabled?[]:Object.keys(CATEGORIES);renderCategories();updateMap();event.currentTarget.textContent=allEnabled?"전체 선택":"전체 해제"});
